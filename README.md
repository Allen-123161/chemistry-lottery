...
<body>
    <!-- 音频元素：修复路径 + 增加ID便于JS控制 -->
    <audio id="bgMusic" src="123.mp3" loop hidden></audio>
    ...
    <script>
        ...
        const magicEntries = [
            ...
            "获得老师肯定",
            "舔狗 一半分数贡献给异性最多的小组",
            "真情吐露 说三遍“我爱化学，化学使我快乐”",
            "共同富裕 最高分给每组100分",
            "味大，无需多盐 分给每组100分",
            "酸碱指示剂--酸 说出3种酸性溶液+300分",
            "酸碱指示剂--碱 说出3种碱性溶液+300分",
            "“红”运当头 小组10元红包碎片(1/2)",
            "取长补短 最高分-200，最低分+200"
        ];
        
        // 初始化音频播放（核心修复）
        function initAudio() {
            const bgMusic = document.getElementById('bgMusic');
            // 方案1：尝试直接播放（部分浏览器支持muted+autoplay）
            bgMusic.muted = true; // 先静音播放，避免自动播放限制
            bgMusic.play().catch(err => {
                // 方案2：页面交互后播放（兜底）
                document.addEventListener('click', () => {
                    bgMusic.muted = false; // 点击后取消静音
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            magicButton.addEventListener('click', function(e) {
                // 确保音频播放（点击交互后强制播放）
                bgMusic.muted = false;
                bgMusic.play().catch(err => console.log('点击后音频播放失败:', err));
